...
def convert_rect(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    rx = _f(elem.get('rx'))
    ry = _f(elem.get('ry'))
    if (rx > 0 or ry > 0) and w > 0 and h > 0:
        r = min(rx or ry, ry or rx)
        shorter = min(w, h)
        adj = int(r / shorter * 100000)
        geom = (
...
def _build_text_runs(
    ...
    for child in elem:
        ...
        if child_tag == 'tspan':
            ...
            # Non-tspan children: extract text content + tail
            t = _normalize_text(''.join(child.itertext()))
            if t:
                runs.append({**parent_attrs, 'text': t})
            if child.tail:
...
def _build_run_xml(
    ...
    lang = 'zh-CN' if any(is_cjk_char(c) for c in text) else 'en-US'
    return f'''<a:r>
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if letter_spacing:
        try:
            spc_val = float(letter_spacing) * 75
