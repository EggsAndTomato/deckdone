...
def _parse_style_attr(elem: ET.Element) -> dict[str, str]:
    """Parse CSS style='...' attribute into individual property key-value pairs."""
...
def _extract_inheritable_styles(elem: ET.Element) -> dict[str, str]:
    ...
    Checks both XML attributes (e.g. fill='#FFF') and CSS style attribute
    (e.g. style='fill:#FFF').
    """
    styles: dict[str, str] = {}
    css_props = _parse_style_attr(elem)
    for attr in INHERITABLE_ATTRS:
        val = elem.get(attr)
        if val is not None:
            styles[attr] = val
        elif attr in css_props:
            styles[attr] = css_props[attr]
    return styles
...
def _get_attr(elem: ET.Element, attr: str, ctx: ConvertContext) -> str | None:
    ...
    Checks XML attribute, then CSS style attribute, then inherited context.
    """
    val = elem.get(attr)
    if val is not None:
        return val
    css_props = _parse_style_attr(elem)
    if attr in css_props:
        return css_props[attr]
