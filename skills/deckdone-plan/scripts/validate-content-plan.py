...
def extract_content_length(content_val):
...
def validate_slide(title, section, catalog_icons=None):
    ...
    if vnp is None:
        errors.append("Missing: Visual Narrative Path field")
    ...
    if tz_val is not None and tz_val != len(zones):
        errors.append(
            f"Total Zones mismatch: declared {tz_val}, found {len(zones)} zones"
        )
    ...
    for label, zs in zones:
        ...
        if ml is None:
            ...
            try:
                ...
                z_errs.append(f"invalid Max Length: {ml} (must be a positive integer)")

        if content_val is not None and ml_val is not None:
            content_len = extract_content_length(content_val)
            if content_len is not None and content_len > ml_val:
                z_errs.append(
                    f"Content length ({content_len}) exceeds Max Length ({ml_val})"
                )
        ...
        if zt is not None and zt == "chart":
            chart_spec = extract_field(zs, "Chart Data")
            if chart_spec is None:
                z_errs.append("chart type zone missing Chart Data specification")
